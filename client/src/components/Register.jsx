...
export default function Register () {
    ...
    function validate (input) {
        ...
        if (input.password) {
            if (!/^(?=\w*\d)(?=\w*[A-Z])(?=\w*[a-z])\S{8,16}$/.test(input.password)) {
                errors.password = "La contraseña debe tener entre 8 y 16 caracteres, al menos un dígito, una minúscula y una mayúscula (No puede tener otros simbolos)!"
            }
    ...
    const have = () => {
        ...
        else if (input.first_name && input.last_name && input.username && input.email && input.password) {
            return false
        }
        else {
            return "e"
    ...
    return (
        <div className="bg-gray-300">
            <form onSubmit={handleSubmit} className="bg-gray-700 rounded-lg shadow sm:max-w-md sm:w-full sm:mx-auto sm:overflow-hidden">
                <div className="flex">
                    <h1>Registrate!</h1>
                </div>
                <div>
                    <label>Nombre</label>
                    <input name="first_name" value={input.first_name} onChange={(e) => handleChange(e)} placeholder="Nombre" className="rounded-lg border-transparent flex-1 appearance-none border border-red-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent"></input>
